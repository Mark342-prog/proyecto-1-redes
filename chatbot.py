    ...
    def send(self, history: list[dict[str, Any]], tools: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for intento in range(1, self.MAX_INTENTOS + 1):
            ...
            try:
                ...
                detail = exc.read().decode("utf-8", errors="replace")
                error = RuntimeError(f"Error de la API de Gemini {exc.code}: {detail}")
                retryable = exc.code in self.RETRYABLE_STATUS
            except (urllib.error.URLError, TimeoutError) as exc:
                reason = getattr(exc, "reason", "tiempo de espera agotado")
                error, retryable = RuntimeError(f"No se pudo conectar con Gemini: {reason}"), True
        ...
        for call in calls:
            if call["name"] not in routes:
                ...
                result = clients[server_name].request(
                    "tools/call", {"name": original_name, "arguments": call.get("args") or {}}
                )
            function_response: dict[str, Any] = {"name": call["name"], "response": {"mcpResult": result}}
            if call.get("id"):
...
def main() -> None:
    ...
    try:
        while True:
            ...
            try:
                print(f"Asistente: {run_gemini_turn(prompt, history, gemini, clients, api_tools, routes)}\n")
            except Exception as exc:
