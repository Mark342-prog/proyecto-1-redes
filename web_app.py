"""Frontend web local y API JSON para el chatbot Gemini + MCP de la farmacia."""
...
from chatbot import GeminiApi, InteractionLog, load_clients, load_dotenv, run_gemini_turn
...
class AppState:
    ...
    def chat(self, session_id: str, message: str) -> str:
        with self.lock:
            ...
            try:
                return run_gemini_turn(message, history, self.gemini, self.clients, self.tools, self.routes)
            except Exception:
...
class WebHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        if self.path == "/api/health":
            self._json(200, {"status": "ok", "provider": "gemini", "mcpServers": list(self.state.clients)})
            return
